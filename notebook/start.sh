...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the notebook username if it exists
    if id notebook &> /dev/null ; then
    ...
    if [[ "$NB_UID" == "$(id -u notebook)" && "$NB_GID" == "$(id -g notebook)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^notebook:/nayvoj:/" > /tmp/passwd
                echo "notebook:x:$(id -u):$(id -g):,,,:/home/notebook:/bin/bash" >> /tmp/passwd
